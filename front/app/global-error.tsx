...
function getErrorContent(message: string): { title: string; description: string; type: ErrorType } {
  const msg = message.toLowerCase();
  if (msg.includes("no se pudo conectar") || msg.includes("verifique su conexión"))
    return { title: "No se pudieron cargar los datos", description: "Los datos del territorio no se pudieron obtener del servidor. Verifique su conexión a internet y recargue la página.", type: "connection" };
  if (msg.includes("tardó demasiado") || msg.includes("timeout"))
    return { title: "La conexión tardó demasiado", description: "La solicitud tardó más de lo esperado. Verifique su conexión a internet e intente de nuevo.", type: "timeout" };
  if (msg.includes("no está disponible temporalmente") || msg.includes("servidor no"))
    return { title: "Algo salió mal", description: "Ocurrió un error al cargar la información. Recargue la página para intentar de nuevo.", type: "server" };
  return { title: "Algo salió mal", description: "Ocurrió un error inesperado. Recargue la página para intentar de nuevo.", type: "unknown" };
}
...
export default function GlobalError({
  ...
  const colorClass = content.type === "timeout" ? "text-amber-400" : "text-rose-400";
        ...
        <div className="min-h-screen bg-[#060606] text-white/90 flex flex-col font-mono p-6 justify-center items-center">
          <div className="max-w-xl w-full bg-[#0b0c10] border border-white/10 rounded-sm p-8 space-y-5 shadow-2xl relative overflow-hidden">
            ...
            <div className={colorClass}>
              <ErrorIllustration type={content.type} />
            </div>
